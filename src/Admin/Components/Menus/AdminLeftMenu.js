...
import '../../css/AdminLeftMenu.css';
import { Link } from "react-router-dom";
import { common } from '../../../common/common';
export default function AdminLeftMenu({ setIsMenuCollapsed, userRole }) {
    setIsMenuCollapsed=common.defaultIfEmpty(setIsMenuCollapsed,()=>{});
    ...
    return (
        <div>
            <ul className={!isCollapsed ? "admin-left-menu" : "admin-left-menu admin-left-menu-small"}>
                ...
                <Link title="Device" to="users"><li onClick={e=>changeMenuSelection("/Device")} className={selectedMenu==="/admin/users"?"selected item":"item"}><i className="fa fa-microchip"></i> {!isCollapsed && ('Users')} </li></Link>
                <Link title="Favourite Device" to="/FavouriteDevice"><li onClick={e=>changeMenuSelection("/FavouriteDevice")} className={selectedMenu==="/FavouriteDevice"?"selected item":"item"}><i className="fas fa-heart"></i> {!isCollapsed && ('Favourite Device')} </li></Link>
                ...
                <Link title="Developers" to="/developers"><li onClick={e=>changeMenuSelection("/developers")} className={selectedMenu==="/developers"?"selected item":"item"}><i className="fas fa-chalkboard-teacher"></i> {!isCollapsed && ('Developers')} </li></Link>
                {
                    ...
                    <>
                        ...
                        <Link title="Dropdown Master" to="/admin/DropdownMaster"><li onClick={e=>changeMenuSelection("/admin/DropdownMaster")} className={selectedMenu==="/admin/DropdownMaster"?"selected item":"item"}><i className="fas fa-stream"></i> {!isCollapsed && ('Dropdown Master')} </li></Link>
                ...
                <li onClick={e=>changeMenuSelection()} title="Sign out" className={selectedMenu===""?"selected item":"item"} onClick={() =>{}}><i className="fas fa-sign-out-alt"></i> {!isCollapsed && ('Sign out')} </li>
            </ul>
